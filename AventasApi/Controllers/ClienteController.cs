...
namespace AventasApi.Controllers
{
    public class ClienteController : ApiController
    {
            ...
            try
            {
                ...
                foreach (var cliente in clientesSinFiltrar)
                {
                    cliente.chequesContabilizados = context.ChequesContabilizados.Where(c => c.Activo && c.CodigoCliente == cliente.Codigo).Select(ch => new ChequesContabilizadosViewModel
                    {
                        ...
                        FechaRecepcion = ch.FechaRecepcion,
                        NumeroRecibo = ch.NumeroRecibo,
                        Banco = ch.Banco,
                        ValorCheque = ch.ValorCheque,
                    ...
                    cliente.AcuerdosXTipoPedido = context.FacturasxCliente.Where(x => x.CodigoCliente == cliente.Codigo && x.Saldo > 0.1M).GroupBy(facCli => facCli.TiposdePedido).Select(asa => new AcuerdosXTipoPedidoViewModel
                    {
                        ...
                        Acuerdos = asa.GroupBy(acu => acu.AcuerdosxCliente).Select(acu => new FacturasXAcuerdosViewModel
                        {
                            ...
                            Facturas = acu.Where(fac => fac.Saldo > 0.1M).OrderBy(facCli => facCli.FechaVencimiento).Select(facCli => new FacturasXClienteViewModel
                            {
                                ...
                                SinDescuento=facCli.SinDescuento,
        ...
        public async Task<IHttpActionResult> GetClientesAgendaPrincipal([FromUri] List<string> clientes)
        {
            try
            {
                using (var ctx = new AVentasEntities())
                {
                    ...
                    {
                        List<ClienteAgendaViewModel> lista = await ctx.Clientes.Where(cli => cli.Habilitado == true && cli.CodigoCliente == client).Select(cli => new ClienteAgendaViewModel
                        {
                            Asesor = cli.CodigoAsesor,
                            EmpresaId = cli.EmpresaId,
                            Codigo = cli.CodigoCliente,
        ...
        public async Task<IHttpActionResult> GetClientesCuenta()
        {
            try
            {
                using (var ctx = new AVentasEntities())
                {
                    ...
                    var carterasCompartidas = asesoresAsignados.Where(c => CodigosAsesorCasa.Contains(c)).ToList();

                    if (carterasCompartidas.Any())
                    {
                        asesoresHabilitados = await ctx.Asesores.Where(x => carterasCompartidas.Contains(x.CodigoAsesor) && x.Activo == true).Select(x => x.CodigoAsesor).ToListAsync();
                    }
                    else if (usuario.FlagTodosAsesores.Value)
                    {
                        asesoresHabilitados = await ctx.Asesores.Where(x => empresas.Contains(x.EmpresaId) && x.Activo == true).Select(x => x.CodigoAsesor).ToListAsync();
                    }
                    else
                    {
                        asesoresHabilitados = await ctx.Asesores.Where(x => asesoresAsignados.Contains(x.CodigoAsesor) && empresas.Contains(x.EmpresaId) && x.Activo == true).Select(x => x.CodigoAsesor).ToListAsync();
                    }
                    ...
                    foreach (var asesor in asesoresHabilitados.Distinct().ToList())
                    {
                        var documentoXCuota = ctx.SP_DocumentosAplicadosXCuotas(asesor).ToList();
                        ...
                        foreach (var cliente in clientes)
                        {
                            cliente.AcuerdosXTipoPedido = ctx.FacturasxCliente.Where(x => x.CodigoCliente == cliente.Codigo && x.Saldo > 0.1M).GroupBy(facCli => facCli.TiposdePedido).Select(asa => new AcuerdosXTipoPedidoViewModel
                            {
                                ...
                                Acuerdos = asa.GroupBy(acu => acu.AcuerdosxCliente).Select(acu => new FacturasXAcuerdosViewModel
                                {
                                    ...
                                    Valor = acu.Key == null ? "0" : (acu.Key.Total ?? 0).ToString(),
                                    Disponible = acu.Key == null ? "0" : (acu.Key.Saldo ?? 0).ToString(),
                                    DescuentoEnAcuerdos = ctx.DescuentoEnAcuerdo.Where(x => x.CodigoDescuento == acu.Key.GrupoDescuento).Select(d => new DescuentoEnAcuerdosViewModel
                                    {
                                        CodigoDescuento = d.CodigoDescuento,
                                        Porcentaje = d.Porcentaje,
                                        Dias = d.Dias,
                                    ...
                                    Facturas = acu.Where(fac => fac.Saldo > 0.1M).OrderBy(facCli => facCli.FechaVencimiento).Select(facCli => new FacturasXClienteViewModel
                                    {
                                        ...
                                        Cuotas = facCli.SubFacturasxCliente.Where(subFac => subFac.FechaMaxDescuento >= DateTime.Today ? (subFac.Saldo - subFac.Descuento) > 0 : subFac.Saldo > 0).OrderBy(subFac => subFac.FechaVencimiento).Select(subFac => new CuotasViewModel
                                        {
                                            ...
                                            IdAcuerdoxCliente = subFac.IdAcuerdoxCliente,
                                            FechaVencimiento = subFac.FechaVencimiento,
                                            FechaMaxDescuento = subFac.AcuerdosxCliente != null ? ctx.CuotasXAcuerdo.FirstOrDefault(c => c.IdAcuerdoVenta == subFac.IdAcuerdoxCliente && c.NumCuota == subFac.NumeroCuota).FechaVencimiento : subFac.FacturasxCliente.FechaMaxDescuento,
                                            Saldo = subFac.Saldo,
                                            SaldoDivisa = subFac.SaldoDivisa,
                                            Descuento = subFac.Descuento,
                                            PendientePago = subFac.PendientePago,
        ...
        public async Task<IHttpActionResult> GetClientesCuenta(string cliente)
        {
            try
            {
                using (var ctx = new AVentasEntities())
                {
                    ...
                    var FechaLimiteFuturo = FechaLimite.AddDays(15);
                    var Recibos = ctx.AnticiposxCliente.Where(r => r.NumPedido != null).Select(a => a.NumPedido).ToList();
                    var documentoXCuota = ctx.SP_DocumentosAplicadosXCuotas(user.UserAccount).ToList();
                    ...
                    clientes[0].DocumentosAplicadosxCuotas = documentoXCuota.Where(x => x.CodigoCliente == cliente).ToList();
        ...
        [HttpGet]
        [Route("~/api/cliente/activos")]
        public async Task<IHttpActionResult> GetClientesActivos()
        {
            try
            {
                using (var ctx = new AVentasEntities())
                {
                    List<ClienteViewModel> clientes = await ctx.Clientes.Where(cli => cli.Habilitado == true).Select(cli => new ClienteViewModel
                    {
                        EmpresaId = cli.EmpresaId,
                        Codigo = cli.CodigoCliente,
                        Nombre = cli.Nombre,
                        ...
                        Latitud = cli.Latitud,
                        Longitud = cli.Longitud,
                        //Correo = ctx.CorreoCliente.FirstOrDefault(x => x.CodigoCliente == cli.CodigoCliente && x.activo).Correo,
                        LimiteCredito = cli.LimiteCredito ?? 0,
                        IgnorarSecuenciaFactura = cli.IgnorarSeqFact,
                        CreditoDisponible = cli.CreditoDisponible ?? 0,
                        GrupoImpuesto = string.IsNullOrEmpty(cli.GrupoImpuesto) ? "CLIENTES" : cli.GrupoImpuesto.ToUpper(),
